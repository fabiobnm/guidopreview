...
export default function Home() {
  ...
  const [openCarousels, setOpenCarousels] = useState(new Set());

  useEffect(() => {
    if (data?.ordineDeiWorks?.[0]?.works?.length > 0) {
      setOpenCarousels(new Set([data.ordineDeiWorks[0].works[0].id]));
    }
  }, [data]);
  ...
  const handleClick = (progettoId) => {
    setOpenCarousels((prev) => {
  ...
  const openModal = (gallery, videos, index) => {
    setCurrentGallery([...gallery, ...videos]);
    ...
    <main style={{ marginTop: '118px', marginBottom: '200px' }}>
      {data.ordineDeiWorks[0].works.map((progetto) => (
        <div className="projectDivWork" key={progetto.id} style={{ marginBottom: 0 }}>
          <h1 onClick={() => handleClick(progetto.id)}
              style={{ color: 'black', fontSize: '20px', padding: '0px', fontFamily: 'MyFontThin', cursor: 'pointer' }}>
            {progetto.nome}
          </h1>
          <div className="caros" style={{
            transition: 'max-height 0.5s ease',
            maxHeight: openCarousels.has(progetto.id) ? 'max-content' : '0',
            marginBlock: openCarousels.has(progetto.id) ? '20px 50px' : '0'
          }}>
            {progetto.galleria.map((image, index) => (
              <div key={index} className="gallery-image-container" style={{ margin: 'auto', marginRight: '0px' }}
                   onClick={() => openModal(progetto.galleria, progetto.video, index)}>
                <Image width={200} height={200} src={image.url} alt={progetto.nome} 
                       style={{ width: '100%', objectFit: 'cover', cursor: 'pointer' }} />
              </div>
            ))}
            {progetto.video.map((video, index) => (
              <div key={index + progetto.galleria.length} className="gallery-image-container" style={{ margin: 'auto', marginRight: '0px' }}
                   onClick={() => openModal(progetto.galleria, progetto.video, index + progetto.galleria.length)}>
                <Image width={200} height={200} src={video.thumbnail?.url} alt={progetto.nome} 
                       style={{ width: '100%', objectFit: 'cover', cursor: 'pointer' }} />
              </div>
            ))}
          </div>
